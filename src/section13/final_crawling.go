...
import(
  ...
  "golang.org/x/net/html" //html 문서 파싱
...
// 첫 번째 방문 (메인페이지) 대상으로 원하는 URL 을 파싱 후 반환하는 함수
func parseMainNodes(n *html.Node) bool {
  // must check for nil values
		if n.DataAtom == atom.A && n.Parent != nil { //atom.A <<- 우리가 찾고자 하는 Tag
...
//파일 스크림 생성 -> 파일명, 옵션, 권한
file, err := os.OpenFile("D:/scrape/"+fn+".txt", os.O_CREATE | os.O_RDWR, os.FileMode(0777))
errCheck(err)
